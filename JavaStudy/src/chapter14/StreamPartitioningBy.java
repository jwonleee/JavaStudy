...
public class StreamPartitioningBy {
	public static void main(String[] args) {
		...
		System.out.println("남자 1등: " + topScoreBySex.get(true)); //리턴값 Optional<Student2>
		System.out.println("여자 1등: " + topScoreBySex.get(false));
		
		Map<Boolean, Student2> topScoreBySex2 = Stream.of(stuArr)
													  .collect(partitioningBy(Student2::isMale,
															   collectingAndThen(maxBy(comparing(Student2::getScore)), Optional::get)));
		System.out.println("남자 1등: " + topScoreBySex2.get(true)); //리턴값 Student2
		...
		Map<Boolean, Map<Boolean, List<Student2>>> failedStuBySex = Stream.of(stuArr)
				                                            .collect(partitioningBy(Student2::isMale, //남자면 true
				                                            		 partitioningBy(s -> s.getScore() <= 100)) //불합격이면 true / List<Student2> 반환
		);
		List<Student2> failMaleStu = failedStuBySex.get(true).get(true);
		List<Student2> failFemaleStu = failedStuBySex.get(false).get(true);
		for(Student2 s : failMaleStu) System.out.println(s);
		for(Student2 s : failFemaleStu) System.out.println(s);
	}
